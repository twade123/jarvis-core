...
| `Handler/boardroom_template.py` + `seat_registry.py` + `model_server_manager.py` | BoardRoom internals (seat conditioning, model assignment, deliberation flow). |
| `boardroom_agent_context.py` + `boardroom_controls.py` + `boardroom_data_poller.py` (root-level) | BoardRoom glue layer — context assembly, runtime controls, UI data polling. |
