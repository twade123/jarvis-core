...
class DatabaseManager:
    """Manages database operations for the handler analyzer."""
    
    def __init__(self, db_path=HANDLER_DB_PATH):
    ...
    def init_database(self):
        ...
        try:
            ...
            tables = {
                ...
                ''',
                'handler_intents': '''
                    CREATE TABLE IF NOT EXISTS handler_intents (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        handler_id INTEGER NOT NULL,
                ...
                'metrics': '''
                    CREATE TABLE IF NOT EXISTS metrics (
                        ...
                        rest_patterns INTEGER NOT NULL,
                        graphql_patterns INTEGER NOT NULL,
                        dynamic_patterns INTEGER NOT NULL,
                        python_patterns INTEGER NOT NULL,
                        applescript_patterns INTEGER NOT NULL,
                        base_urls INTEGER NOT NULL,
                        <healthcare>_specific_queries INTEGER NOT NULL,
                        <healthcare>_specific_mutations INTEGER NOT NULL,
                        <healthcare>_specific_fields INTEGER NOT NULL,
                        <healthcare>_specific_endpoints INTEGER NOT NULL,
                        <healthcare>_specific_base_urls INTEGER NOT NULL,
                        <healthcare>_specific_parameters INTEGER NOT NULL,
                        <healthcare>_specific_api_errors INTEGER NOT NULL,
                        <healthcare>_specific_validation_errors INTEGER NOT NULL,
                        <healthcare>_specific_operations INTEGER NOT NULL,
                        graphql_queries INTEGER NOT NULL DEFAULT 0,
                        graphql_mutations INTEGER NOT NULL DEFAULT 0,
                        graphql_fields INTEGER NOT NULL DEFAULT 0,
                        graphql_types INTEGER NOT NULL DEFAULT 0,
                        graphql_fragments INTEGER NOT NULL DEFAULT 0,
                        graphql_interfaces INTEGER NOT NULL DEFAULT 0,
                        graphql_unions INTEGER NOT NULL DEFAULT 0,
                        graphql_inputs INTEGER NOT NULL DEFAULT 0,
                        graphql_directives INTEGER NOT NULL DEFAULT 0,
                        graphql_subscriptions INTEGER NOT NULL DEFAULT 0,
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        FOREIGN KEY (handler_id) REFERENCES handlers(id)
                    )
                ''',
                ...
                'handler_analysis': '''
                    CREATE TABLE IF NOT EXISTS handler_analysis (
                        ...
                        python_calls_count INTEGER DEFAULT 0,
                        applescript_commands_count INTEGER DEFAULT 0,
                        semantic_matches_count INTEGER DEFAULT 0,
                        training_data TEXT,
...
class EndpointDiscovery:
    """Main class for discovering and analyzing endpoints in handler files."""
    
    def __init__(self):
        """Initialize the endpoint discovery system."""
        try:
            # Initialize database connection
            self.conn = sqlite3.connect(HANDLER_DB_PATH, isolation_level=None)
            self.init_database()
            
            # Initialize spaCy for text similarity - using large model for better word vectors
            try:
            ...
            # Initialize pattern and intent storage
            self.patterns = defaultdict(list)
            self.loaded_patterns = defaultdict(list)
            self.loaded_intents = {}
            self.intents_by_category = {}
            
            # Initialize default categories
            default_categories = ['coding', 'code_exec', 'workspace', 'gpt_swarm', 'agent_builder', 
                                'board_room', 'ai_queries', 'data_validation', 'browser', 'document', 
                                'spreadsheet', 'swarm', 'claude', 'file_sharing', 'news', 'info', 
                                'weather', 'tv', 'movies', 'system', 'base']
                                
            # Corrected handlers mapping with proper file names
            self.handlers = {
            ...
            for category in default_categories:
                ...
                if category not in self.loaded_intents:
                    self.loaded_intents[category] = []  # Initialize as list instead of dict
            
            # Add logging for pattern and intent coordination
            logging.info("=== Coordinating Patterns and Intents with Handlers ===")
            for handler_file, pattern_files in self.handlers.items():
                logging.info(f"Processing handler: {handler_file}")
                for pattern_file in pattern_files:
                    logging.info(f"Associated pattern file: {pattern_file}")
            
            # Load existing patterns
            self._load_patterns()
            
            logging.info("Endpoint discovery system initialized successfully")
        except Exception as e:
            logging.error(f"Failed to initialize endpoint discovery: {e}")
    ...
    def _extract_patterns(self, content: str) -> Dict[str, Set[str]]:
        """Extract patterns from handler content."""
        patterns = {
            'rest': set(),
            'graphql': set(),
            'dynamic': set(),
    ...
    def match_patterns_to_intents(self, patterns: List[Dict], handler_type: str):
        ...
        try:
            ...
            # Set threshold based on handler type - lowered thresholds
    ...
    def analyze_handler(self, handler_file: str) -> Tuple[str, List[Dict], List[Dict]]:
        ...
        try:
            ...
            from pattern_intent_mapper import PatternIntentMapper
            
            # Initialize the global pattern mapper
            pattern_mapper = PatternIntentMapper(db_connection=self.conn)
            
            # Map patterns using the global mapper
            intent_matches = pattern_mapper.map_patterns_to_intents(
                patterns=patterns,
...
def main():
    try:
        ...
        analyzer = EndpointDiscovery()
        
        # Initialize database and tables
        db_manager = DatabaseManager(HANDLER_DB_PATH)
        db_manager.init_database()  # Ensure tables are created
        analyzer.conn = db_manager.conn
        
        # Step 1: Load all patterns first
        logging.warning("\n=== LOADING PATTERNS ===")
        pattern_counts = analyzer.load_patterns('Patterns')
        
        # Step 2: Load all intents
        logging.warning("\n=== LOADING INTENTS ===")
        analyzer.load_intents('Intents')
        ...
        # Step 3: Process each handler in sequence
        for handler_file, handler_type in supported_handlers:
            try:
                ...
                # Analyze handler and get results
                result = analyzer.analyze_handler(handler_path)
                if result:
                    handler_type, patterns, intent_matches = result
                    ...
                    logging.warning(f"No patterns found for {handler_type}")
                    failed += 1
